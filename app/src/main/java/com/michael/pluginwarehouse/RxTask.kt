...
object RxTask {
     ...
     * @param uiTask
     */
    fun doInUIThread(doInUIThread: () -> Unit) {
        doInUIThreadDelay(doInUIThread, 0, TimeUnit.MILLISECONDS)
    }
     ...
     * @param timeUnit
     */
    fun doInUIThreadDelay(doInUIThread: () -> Unit, time: Long, timeUnit: TimeUnit?) {
        Observable.just(1)
            .delay(time, timeUnit)
    ...
    fun <T> executeRxTaskDelay(
        ...
        Observable.create(ObservableOnSubscribe<Any> {
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { tResult?.let { doInUIThread(it) } }
    }
}